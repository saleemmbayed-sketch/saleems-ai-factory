#!/usr/bin/env python3
"""Build the Hermes lazy-router plugin for Saleem's AI Factory agents.
...
skills.external_dirs, which advertises every AI Factory agent in Hermes' initial
skill catalog.
"""
...
def plugin_yaml() -> str:
    return textwrap.dedent(
        ...
        name: {PLUGIN_NAME}
        version: 1.0.0
        description: Lazy search/load/delegate router for Saleem's AI Factory agent roster.
        provides_tools:
...
def init_py() -> str:
    return r'''"""Hermes plugin: lazy router for Saleem's AI Factory agents."""
from __future__ import annotations
...
def _specialist_prompt(agent: dict[str, Any], task: str = "") -> str:
    ...
    return (
        f"Use the following AI Factory specialist context for this turn. "
...
SEARCH_DESCRIPTION = (
    "Search Saleem's AI Factory's on-disk specialist agent roster without loading all "
    "agents into the prompt. Use this when the user asks for an AI Factory/Data "
...
READ_DESCRIPTION = (
    "Read one AI Factory specialist by slug or name. Returns metadata by default "
...
PROMPT_DESCRIPTION = (
    "Load a selected AI Factory specialist as a prompt block for the current task. "
...
DELEGATE_DESCRIPTION = (
    "Delegate a task to one selected AI Factory specialist through Hermes' "
...
def readme(agent_count: int) -> str:
    return textwrap.dedent(
        f"""
        # Hermes AI Factory Router Plugin
        ...
        small fixed tool surface at startup, while the complete AI Factory roster is
        ...
        When a Hermes project needs AI Factory specialists, explicitly ask Hermes to use
        the `{PLUGIN_NAME}` plugin/router and load only the specialists needed for
        the current phase. Do not ask Hermes to install or preload the full AI Factory
        roster as skills.

        Recommended project instruction:

        ```text
        Use the agency-agents-router plugin. Search the AI Factory roster for the right
        ...
        full AI Factory roster and do not add agency-agents to skills.external_dirs.
        ```
        ...
        relevant AI Factory specialists. Search first, then delegate to selected agents
        ...
        arguments, regenerate and reinstall the plugin from the latest Saleem's AI Factory
