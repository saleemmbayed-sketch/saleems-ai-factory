...
# install.sh -- Install Saleem's AI Factory agents into your local agentic tool(s).
#
...
screen_tools() {
  ...
  SEL_TITLE="Saleem's AI Factory · Installer  —  1/3 · Tools"
...
screen_teams() {
  ...
  SEL_TITLE="Saleem's AI Factory · Installer  —  2/3 · Teams"
...
screen_review() {
  ...
  while true; do
    local buf="" m
    # pager
    buf+="  ${C_BOLD}${C_CYAN}${BX_TL}${BX_H}${BX_H} Saleem's AI Factory · Installer  —  3/3 · Review $(repeat "$BX_H" 28)${BX_TR}${C_RESET}"$'\n'
    # description
...
main() {
  ...
  if $use_interactive && interactive_wizard; then
    ...
    header "Saleem's AI Factory -- Scanning for installed tools..."
    printf "\n"
    local t
  ...
  if $DRY_RUN; then
    ...
    printf "\n"; header "Saleem's AI Factory -- Dry run (nothing written)"
    printf "  Tools:   %s\n" "${SELECTED_TOOLS[*]}"
    if $SELECTION_ACTIVE; then
  ...
  printf "\n"
  header "Saleem's AI Factory -- Installing agents"
  printf "  Repo:       %s\n" "$REPO_ROOT"
